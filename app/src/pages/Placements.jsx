...
const PlaceMent = () => {
    ...
    const EduFormRow = ({ children }) => (
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, mb: 3 }}>
            {React.Children.map(children, (child) => (
                <Box sx={{ flex: 1, minWidth: '250px', display: 'flex', alignItems: 'flex-start', '& > *': { width: '100%' } }}>
                    {child}
                </Box>
            ))}
        </Box>
    ...
    const eduHandleSubmit = async () => {
      ...
      const res = await Api.post("/placements", eduFormData);
...
{/*header*/}
            <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Typography variant="h4" fontWeight="800" color={EDU_COLORS.primary} sx={{ letterSpacing: "-0.5px", fontSize: { xs: "1.75rem", md: "2.125rem" } }}>
                    Career Launchpad
                </Typography>
                <Box sx={{ display: 'flex', gap: 3 }}>
                    {['THIS WEEK', 'THIS MONTH', '6 MONTHS'].map((tab) => (
                        <Typography key={tab} sx={{ fontSize: '0.8rem', fontWeight: 'bold', color: EDU_COLORS.primary, cursor: 'pointer', '&:hover': { opacity: 0.7 } }}>
                            {tab}
                        </Typography>
                    ))}
                </Box>
...
{/* Placement Summary Cards */}
            <Grid container spacing={3} sx={{ mb: 4 }}>
                <Grid item xs={12} md={3}>
                    <Paper elevation={0} sx={{ p: 3, borderRadius: 4, position: 'relative' }}>
...
{/* Placement Company Details Table */}
            ...
            <TableContainer component={Paper} sx={{ border: "2px solid #1f4d3a", overflow: 'hidden', elevation: 0, mb: 4 }}>
                <Table>
                    <TableHead sx={{ backgroundColor: EDU_COLORS.primary }}>
                        <TableRow>
                            <TableCell sx={{ color: 'white', fontWeight: 'bold' }}>Company</TableCell>
                            <TableCell sx={{ color: 'white', fontWeight: 'bold' }}>Job Role</TableCell>
                            <TableCell sx={{ color: 'white', fontWeight: 'bold' }}>Students Placed</TableCell>
                            <TableCell sx={{ color: 'white', fontWeight: 'bold' }}>Package (LPA)</TableCell>
